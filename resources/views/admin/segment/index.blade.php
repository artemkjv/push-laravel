...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <form action="{{ route('admin.segment.index', ['userId' => $user->id]) }}" class="d-flex filters-wrapper mb-2">
                        <a href="{{ route('admin.segment.create', ['userId' => $user->id]) }}" class="btn btn-primary">Add Segment</a>
                            ...
                            <table class="table table-bordered table-hover text-nowrap">
                                ...
                                @foreach($segments as $segment)
                                    <tr>
                                        <td>{{ $segment->id }}</td>
                                        <td>{{ $segment->name }}</td>
                                        <td>{{ $segment->push_users_count }}</td>
                                        <td class="d-flex justify-content-around">
                                            <a href="{{ route('admin.segment.edit', ['userId' => $user->id, 'id' => $segment->id]) }}"
                                               class="btn btn-info btn-sm float-left mr-1">
                                                <ion-icon name="create" class="action-icon"></ion-icon>
                                            </a>

                                            <form
                                                action="{{ route('admin.segment.destroy', ['userId' => $user->id, 'id' => $segment->id]) }}"
                    ...
                    {{ $segments->links("pagination::bootstrap-4") }}
