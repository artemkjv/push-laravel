...
@section('content')
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Create Temporary Token</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="container-fluid">
            <form role="form" enctype="multipart/form-data" method="post" action="{{ route('apiToken.store') }}">
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" min="{{ \Carbon\Carbon::now()->format('Y-m-d') }}"
                                   value="{{ old('expires_at') }}" id="expires-at" name="expires_at">
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select multiple class="tokenize2" id="pages" name="actions[]" aria-label="Actions">
                                @foreach($apiPages as $apiPage)
                                    <option value="{{ $apiPage->id }}">{{ $apiPage->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
