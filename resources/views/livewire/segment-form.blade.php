    ...
    @if(count($filterTypes))
        <div class="filter-types col-xl-6 col-sm-12">
            @foreach($filterTypes as $filterType)
                <a type="button" wire:click.prevent="addFilterListener({{ $filterType->id }})"
                   class="filter-type mt-2 non-underline">
    ...
    @if($filters)
        <div class="filters col-xl-6 col-sm-12">
                ...
                <div class="filter-group">
                    @foreach($filterGroup as $filterIndex => $filter)
                        <div class="filter">
                            ...
                            <input type="hidden"
                                   name="group[{{ $filterGroupIndex }}][{{ $filterIndex }}][filter_type_id]"
                                   value="{{ $filter['filter_type']['id'] }}">
                            <div class="filter-values">
                                ...
                                @if($filter['entities'])
                                    <select class="form-control"
                                            name="group[{{ $filterGroupIndex }}][{{ $filterIndex }}][value]">
                                        @foreach($filter['entities'] as $entity)
                                            <option @if($filter['value'] == $entity['id']) selected
                                                    @endif value="{{ $entity['id'] }}">
                                                {{ $entity['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                @else
                                    @if($filter['filter_type']['key_allowed'])
                                        <input type="text" class="form-control" value="{{ $filter['tag_key'] }}"
                                               name="group[{{ $filterGroupIndex }}][{{ $filterIndex }}][tag_key]">
                                    @endif
            ...
            <button type="button" class="btn btn-secondary"
                    wire:click.prevent="addGroup()" style="display: inline-block;">
                Add or
            </button>
        </div>
    @endif
